
var id = d3.select("#selDataset");

// demographic information
var demTable = d3.select("#sample-metadata");

// bar graph
var barGraph = d3.select("#bar");

// bubble graph
var bubbleGraph = d3.select("bubble");

// select the gauge chart div
var guageGraph = d3.select("gauge");

        // creating empty list
var otu_ids = [];
var otu_labels = [];
 var sample_values = [];

// create a function to initially populate dropdown menu with IDs and draw charts by default (using the first ID)
function init() {

    // reset any previous data
    newData();

    // read in samples from JSON file
    d3.json("samples.json").then((data => {

// Populating dropdown menu from dataset

        //  use a forEach to loop over each name in the array data.names to populate dropdowns with IDs
        data.names.forEach((name => {
            var option = id.append("option");
            option.text(name);
        })); // close forEach

        // get the first ID from the list for initial charts as a default
        var initId = id.property("value")

        // plot charts with initial ID
        plotCharts(initId);

    })); // close .then()

} // close init() function

// create a function to reset divs to prepare for new data
function newData() {

    // clearing input data

    demTable.html("");
    barGraph.html("");
    bubbleGraph.html("");
    guageGraph.html("");

}; 

// reading JSON

function plotCharts(id) {

    // read in the JSON data
    d3.json("samples.json").then((data => {

// populating demographic table

        var demographicData = data.metadata.filter(participant => participant.id == id)[0];

        // get the wash frequency for gauge chart later
        var wfreq = demographicData.wfreq;

        // Iterate through each key and value in the metadata
        Object.entries(demographicData).forEach(([key, value]) => {

            var list = demTable.append("ul");
            list.attr("class", "list-group list-group-flush");

            // append a li item to the unordered list tag
            var listedItems = list.append("li");

            // change the class attributes of the list item for styling
            listedItems.attr("class", "list-group-item p-1 demo-text bg-transparent");

            // add the key value pair from the metadata to the demographics list
            listedItems.text(`${key}: ${value}`);

        }); 

    // getting data for plotting 

        
        var singleFilter = data.samples.filter(sample => sample.id == id)[0];



        // Iterate through each key and value in the sample to retrieve data for plotting
        Object.entries(singleFilter).forEach(([key, value]) => {

            switch (key) {
                case "otu_ids":
                    otu_ids.push(value);
                    break;
                case "sample_values":
                    sample_values.push(value);
                    break;
                case "otu_labels":
                    otu_labels.push(value);
                    break;
                    // case
                default:
                    break;
            } // close switch statement

        }); // close forEach

        // slice and reverse the arrays to get the top 10 values, labels and IDs
        var topTentuIds = otu_ids[0].slice(0, 10).reverse();
        var topTentuLabels = otu_labels[0].slice(0, 10).reverse();
        var topSample = sample_values[0].slice(0, 10).reverse();

        // use the map function to store the IDs with "OTU" for labelling y-axis
        var topTentuIdsFormatted = topTentuIds.map(otuID => "OTU " + otuID);

        var traceBar = {
            x: topSample,
            y: topTentuIdsFormatted,
            text: topTentuLabels,
            type: 'bar',
            orientation: 'h',
            marker: {
                color: 'rgb(29,145,192)'
            }
        };

        // create the data array for plotting
        var dataBar = [traceBar];

        // define the plot layout
        var layoutBar = {
            height: 500,
            width: 600,
            font: {
                family: 'Sans-serif'
            },
            hoverlabel: {
                font: {
                    family: 'Sans-serif'
                }
            },
            title: {
                text: `<b>Top OTUs ${id}</b>`,
                font: {
                    size: 18,
                    color: 'rgb(34,94,168)'
                }
            },
            xaxis: {
                title: "<b>Sample values<b>",
                color: 'rgb(34,94,168)'
            },
            yaxis: {
                tickfont: { size: 14 }
            }
        }


        // plot the bar chart to the "bar" div
        Plotly.newPlot("bar", dataBar, layoutBar);

   
        // plotting buble chart
    

        var bubbleTrace = {
            x: otu_ids[0],
            y: sample_values[0],
            text: otu_labels[0],
            mode: 'markers',
            marker: {
                size: sample_values[0],
                color: otu_ids[0],
                colorscale: 'YlGnBu'
            }
        };

        // create the data array for the plot
        var dataBub = [bubbleTrace];

        // define the plot layout
        var layoutBub = {
            font: {
                family: 'Sans-serif'
            },
            hoverlabel: {
                font: {
                    family: 'Sans-serif'
                }
            },
            xaxis: {
                title: "<b>OTU Id</b>",
                color: 'rgb(34,94,168)'
            },
            yaxis: {
                title: "<b>Sample Values</b>",
                color: 'rgb(34,94,168)'
            },
            showlegend: false,
        };

        // plot the bubble chat to the appropriate div
        Plotly.newPlot('bubble', dataBub, layoutBub);

// Bonus

        if (wfreq == null) {
            wfreq = 0;
        }

        // create an indicator trace for the gauge chart
        var gauge = {
            domain: { x: [0, 1], y: [0, 1] },
            value: wfreq,
            type: "indicator",
            mode: "gauge",
            gauge: {
                axis: {
                    range: [0, 9],
                    tickmode: 'linear',
                    tickfont: {
                        size: 15
                    }
                },
                bar: { color: 'rgba(8,29,88,0)' },
                steps: [
                    { range: [0, 1], color: 'rgb(255,255,217)' },
                    { range: [1, 2], color: 'rgb(237,248,217)' },
                    { range: [2, 3], color: 'rgb(199,233,180)' },
                    { range: [3, 4], color: 'rgb(127,205,187)' },
                    { range: [4, 5], color: 'rgb(65,182,196)' },
                    { range: [5, 6], color: 'rgb(29,145,192)' },
                    { range: [6, 7], color: 'rgb(34,94,168)' },
                    { range: [7, 8], color: 'rgb(37,52,148)' },
                    { range: [8, 9], color: 'rgb(8,29,88)' }
                ]
            }
        };

        // determine angle for each wfreq segment on the chart
        var angle = (wfreq / 9) * 180;

        // calculate end points for triangle pointer path
        var degrees = 180 - angle,
            radius = .8;
        var radians = degrees * Math.PI / 180;
        var x = radius * Math.cos(radians);
        var y = radius * Math.sin(radians);

        
        var mainPath = 'M -.0 -0.025 L .0 0.025 L ',
            cX = String(x),
            cY = String(y),
            pathEnd = ' Z';
        var path = mainPath + cX + " " + cY + pathEnd;

        gaugeColors = ['rgb(8,29,88)', 'rgb(37,52,148)', 'rgb(34,94,168)', 'rgb(29,145,192)', 'rgb(65,182,196)', 'rgb(127,205,187)', 'rgb(199,233,180)', 'rgb(237,248,217)', 'rgb(255,255,217)', 'white']

        // create a trace to draw the circle where the needle is centered
        var needle = {
            type: 'scatter',
            showlegend: false,
            x: [0],
            y: [0],
            marker: { size: 35, color: '850000' },
            name: wfreq,
            hoverinfo: 'name'
        };

        var dataGauge = [gauge, needle];
      
        var layoutGauge = {

            // draw needle
            shapes: [{
                type: 'path',
                path: path,
                fillcolor: '850000',
                line: {
                    color: '850000'
                }
            }],
            font: {
                family: 'Sans-serif'
            },
            hoverlabel: {
                font: {
                    family: 'Sans-serif',
                    size: 16
                }
            },
            title: {
                text: `<b>Test Subject ${id}</b><br><b>Belly Button Washing Frequency</b><br><br>Scrubs per Week`,
                font: {
                    size: 18,
                    color: 'rgb(34,94,168)'
                },
            },
            height: 500,
            width: 500,
            xaxis: {
                zeroline: false,
                showticklabels: false,
                showgrid: false,
                range: [-1, 1],
                fixedrange: true // disable zoom
            },
            yaxis: {
                zeroline: false,
                showticklabels: false,
                showgrid: false,
                range: [-0.5, 1.5],
                fixedrange: true // disable zoom
            }
        };

        // plot the gauge chart
        Plotly.newPlot('gauge', dataGauge, layoutGauge);


    })); 

}; 

function newID(id) {

    // reset the data
    newData();

    // plot the charts for this id
    plotCharts(id);


} // close optionChanged function

// call the init() function for default data
init();